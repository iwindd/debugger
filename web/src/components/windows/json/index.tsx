      ...
      <div className="json-data">
        ...
        <article>
          {(selectedCategory && (
            <ul>
              {jsonData
                .filter((data) => data.invokingResource === selectedCategory)
                .filter((data) => data.id.toLocaleLowerCase().includes(search.toLocaleLowerCase()))
                .sort((a, b) =>sort ? b.lastUpdated - a.lastUpdated : a.lastUpdated - b.lastUpdated)
                .map((data) => (
                  <li
                    key={data.id}
                  ...
                  >
                    <header>
                      {data.id}:<span>{new Date(data.lastUpdated).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </header>
                    <pre>
                      <JsonView
                        objectSortKeys={true}
